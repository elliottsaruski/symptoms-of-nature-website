function AboutPage() {
  return (
    <div id="home-about-wrapper">
      <h2>sYmptoMs oF NatURe</h2>
      <span>
        Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quos nihil
        debitis, saepe distinctio molestias non molestiae tempore dignissimos
        nulla eveniet, dolor, consequatur tempora enim voluptatem! Velit fuga
        illum consequatur quae! Lorem ipsum dolor sit, amet consectetur
        adipisicing elit. Quos nihil debitis, saepe distinctio molestias non
        molestiae tempore dignissimos nulla eveniet, dolor, consequatur tempora
        enim voluptatem! Velit fuga illum consequatur quae!
      </span>
      <hr />
      <div className="home-content-wrapper">
        <div className="about-img-wrapper-1">
          <div className="about-img-placeholder"></div>
          <div className="about-img-placeholder"></div>
          <div className="about-img-placeholder"></div>
          <div className="about-img-placeholder"></div>
          <div className="about-img-placeholder"></div>
        </div>
        <div className="span-section-wrapper">
          <span>
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quos nihil
            debitis, saepe distinctio molestias non molestiae tempore
            dignissimos nulla eveniet, dolor, consequatur tempora enim
            voluptatem! Velit fuga illum consequatur quae! Lorem ipsum dolor
            sit, amet consectetur adipisicing elit. Quos nihil debitis, saepe
            distinctio molestias non molestiae tempore dignissimos nulla
            eveniet, dolor, consequatur tempora enim voluptatem! Velit fuga
            illum consequatur quae!
          </span>
          <div className="about-img-secondary"></div>
        </div>
        <div className="about-img-wrapper-2">
          <div className="about-img-placeholder2"></div>
          <div className="about-img-placeholder2"></div>
          <div className="about-img-placeholder2"></div>
          <div className="about-img-placeholder2"></div>
          <div className="about-img-placeholder2"></div>
          <div className="about-img-placeholder2"></div>
          <div className="about-img-placeholder2"></div>
          <div className="about-img-placeholder2"></div>

          <div className="about-img-placeholder2"></div>
        </div>
        <span>
          Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quos nihil
          debitis, saepe distinctio molestias non molestiae tempore dignissimos
          nulla eveniet, dolor, consequatur tempora enim voluptatem! Velit fuga
          illum consequatur quae! Lorem ipsum dolor sit, amet consectetur
          adipisicing elit. Quos nihil debitis, saepe distinctio molestias non
          molestiae tempore dignissimos nulla eveniet, dolor, consequatur
          tempora enim voluptatem! Velit fuga illum consequatur quae!
        </span>
      </div>
    </div>
  );
}

export default AboutPage;
